...
{
	double k_elastic = 5;
	return val*val*val*val - k_elastic*val*val/2.0;
}

double external_potential_prime(double val)
{
	double k_elastic = 5;
	return 4*val*val*val - k_elastic*val;
}

double external_potential_second(double val)
{
	double k_elastic=1;
	return 12*val*val - k_elastic;
}
...
double variationalWaveFunction(double v)
{
    ...
    // optimal params mu = 0.81 and sigma=0.616
}

double variationalWaveFunction_second(double x)
{
	//return 0;
	return 1./(2*sigma*sigma) - ((x + mu)*(x + mu)*exp(- (x + mu)*(x + mu)/(2*sigma*sigma)) + (x - mu)*(x - mu)*exp(- (x - mu)*(x - mu)/(2*sigma*sigma)))/(2*pow(sigma,4)*(exp(- (x + mu)*(x + mu)/(2*sigma*sigma)) + exp(- (x - mu)*(x - mu)/(2*sigma))));
}
...
void brownianBridge()
{
	...
	// metropolis. Note that the kinetic part has been sampled exactly, thus only the
...
void finalizePotentialEstimator()
{
    ...
    file_name = "QMC_1D_data/potential_opt"+temp_name+".txt";
...
{
    std::string file_name;
    file_name = "QMC_1D_data/kinetic_opt"+temp_name+".txt";
...
{
    std::string file_name;
    file_name = "QMC_1D_data/probability_opt"+temp_name+".txt";
...
void readInput(int argc, char *argv[])
{
    ...
    if (argc > 1){               // added to simplify sequential launch
        //temperature = 0.25 + 2.25*atof(argv[1])/atof(argv[2]);
        imaginaryTimePropagation = atof(argv[1]);
    }
    
    //temp_name = to_string(temperature);
    temp_name = "_" + to_string(imaginaryTimePropagation);
