...
//Read input informations
  ...
  if (arg > 3){       // passed by command line for long sims
      nblk = atoi(argv[2]);
      nstep = atoi (argv[3]);
      restart = atoi (argv[4]);
      long_ = atoi(argv[5]);
...
//Prepare arrays for measurements
  ...
  n_props = 2; //Number of observables
    ...
    if (restart==1)  // easy way to remember starting configuration
...
void Measure()
{
    ...
    for (int i=0; i<npart-1; ++i)
    {
        for (int j=i+1; j<npart; ++j)
        {
            ...
            bin = floor(dr/bin_size) + igofr; // first two positions are dedicated to V and p
            walker[bin]+=2.;    // two particles are added due
                                // to double sum
            
            if(dr < rcut) // possibility of interaction
            {
...
//Potential energy per particle and pressure
    Epot << setw(wd) << iblk <<  setw(wd) << stima_pot << setw(wd);
    Pres << setw(wd) << iblk <<  setw(wd) << stima_pres << setw(wd);
#ifdef OUT_AVE
    Epot << glob_av[iv]/(double)iblk << setw(wd) << err_pot << endl;
    Pres << glob_av[iw]/(double)iblk << setw(wd) << err_press << endl;
#endif
    ...
    for(int i=igofr; i < n_props; ++i){
        ...
        if (iblk == nblk){   // saves final g(r) w/ errors
            dV = pow(r+bin_size/2,3)-pow(r-bin_size/2,3);
            Gave << std::setw(wd) << r
            << std::setw(wd) << glob_av[i]/(double)iblk/norm/dV
...
void ConfFinal(void)
{
    ...
    cout << file_name << " Print final configuration to file config.final " << endl << endl;
